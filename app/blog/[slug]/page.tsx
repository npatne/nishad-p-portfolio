    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="relative h-64 md:h-96">
        ...
        <div className="absolute inset-0 flex items-end p-6 md:p-10">
          <div className="md:max-w-5xl lg:max-w-7xl mx-auto w-full">
            {" "}
            {/* Constrain width */}
            <Link
              href="/blog"
      ...
      <div className="px-6 md:px-10 py-12 md:max-w-5xl lg:max-w-7xl mx-auto space-y-12">
        {" "}
        {/* Add space between sections */}
        {post.content.map((section, index) => (
          <section key={section.id || index} id={section.id}>
            {" "}
            {/* Add section tag and ID */}
            {/* Optionally render the section title if it exists */}
            {section.title && (
              <h2 className="text-2xl md:text-3xl font-bold mb-6 mt-8 first:mt-0">
                {" "}
                {/* Add styling for title */}
                {section.title}
              </h2>
            )}
            <ProjectSection
              ...
              imageAlt={
                section.imageAlt || `${post.title} - Section ${index + 1}`
              }
      ...
      {/* Footer section */}
      <div className="border-t border-gray-200 mt-12 py-8 text-center text-muted-foreground">
        <p className="mb-4">Thanks for reading!</p>
        <p className="mb-2">
          I am always available for a chat, will be active on{" "}
          <a
            href="https://www.linkedin.com/in/nishad-patne"
            target="_blank"
        ...
        <p className="flex items-center justify-center text-2xl">
          <span className="text-xs">bye</span>
          <span
            className="
                  mx-2 
                  text-3xl 
